...
> This article is the reference for keys, webhooks, and connectors. To push orders in for the first time, follow [push orders in over the API](/help/guides/receive-orders-over-api).
...
Delivery endpoints are configured per supplier, not in one central list. **Suppliers** shows each supplier's delivery channel, and opening a supplier's **Delivery** tab is where you set the endpoint, its credentials, and test-fire it — see [setting up delivery and test-fire](/help/delivery-setup).
