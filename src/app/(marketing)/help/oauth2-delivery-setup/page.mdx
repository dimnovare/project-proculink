...
> **Which plan you need.** Everything here is on the HTTP delivery channel, which is {requiresPlan("webhookDelivery")}. Saving an HTTP delivery config on a cheaper plan is refused at the point of saving, so the OAuth2 fields below are not reachable until then. SFTP, FTPS and email delivery are on every plan and need none of this.

> This article is the OAuth2 reference. For the delivery set-up it belongs to, follow [set up delivery to a supplier and test-fire it](/help/guides/set-up-supplier-delivery).
...
Configure these under **Partners → Suppliers → Delivery**, after picking HTTP and the OAuth2 fetch-token method. Each is one line; a concrete example follows.
