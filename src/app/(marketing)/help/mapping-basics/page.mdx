...
For logic a manipulator chain can't express — calculated values like `{{ line.Quantity * line.UnitPrice }}`, or combining order and line fields — the [output mapping editor](/help/output-mapping-editor) accepts free-form Scriban expressions and even a whole-document template. That's the escape hatch for a supplier's exact required structure; most mappings never need it.
