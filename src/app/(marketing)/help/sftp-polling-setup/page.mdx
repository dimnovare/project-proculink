...
> This article is the reference for the individual settings. For the set-up in order, follow [pull order files from an SFTP folder](/help/guides/receive-orders-by-sftp).
...
- **Credentials** — a **username and password**. Credentials are encrypted at rest with AES-256-GCM and are never shown again once saved; leaving the password blank on a later edit keeps the stored one. Key-based authentication is available for *delivery* over SFTP, not for pull ingestion — see [SFTP and FTPS delivery keys](/help/sftp-ftps-delivery-keys).
- **Default supplier** — the supplier that imported files are attributed to, since a raw file drop carries no ProcuLink routing.

The cadence is not configurable: ProcuLink checks the folder every few minutes while polling is switched on.
