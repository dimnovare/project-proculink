...
- **Two known gaps in the document itself.** The supplier party name is emitted as an internal identifier rather than the supplier's registered name, and no `cbc:EndpointID` is emitted for either party. Both are mandatory under BIS 3, and neither is fixed today.
