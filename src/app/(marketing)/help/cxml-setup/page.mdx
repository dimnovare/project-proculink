...
> **Which plan you need.** Parsing an inbound cXML order is on every plan. Delivering cXML **out** to a supplier is {requiresPlan("cxml")} — choosing cXML as a supplier's output format on a cheaper plan is refused when you save the delivery config, not later when you send. cXML rides on the HTTP channel, which is separately {requiresPlan("webhookDelivery")}. The rest of this article is the reference for the credentials; [plans and quotas](/help/billing-faq) covers the ladder.
