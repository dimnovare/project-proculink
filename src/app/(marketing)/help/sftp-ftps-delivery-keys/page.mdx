...
When a supplier receives orders as files on their server rather than through an API, ProcuLink drops the transformed order file over **SFTP** or **FTPS**. Configure it per supplier under **Partners → Suppliers → Delivery**. This article covers the two ways to authenticate and the file-placement settings that trip people up.

> This article is the reference for keys and certificates. For the delivery set-up they belong to, follow [set up delivery to a supplier and test-fire it](/help/guides/set-up-supplier-delivery).
