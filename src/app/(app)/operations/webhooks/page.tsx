  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden" style={{ background: "#F6F7FA" }}>
      <div className="flex flex-col items-start gap-3 px-4 py-4 sm:px-6 sm:flex-row sm:items-end sm:gap-4 flex-shrink-0" style={{ borderBottom: "1px solid #E2E6EE", background: "#FFFFFF" }}>
        ...
        <button
          onClick={() => setSelected({ id: "new", url: "", events: ["crossing.sent"], status: "ok", deliveries24h: 0, lastDelivery: "never" })}
          ...
          {WEBHOOKS.map((w) => (
            <div key={w.id} className="rounded-[8px]" style={{ background: "#FFFFFF", border: "1px solid #E2E6EE", boxShadow: "0 1px 3px rgba(11,26,47,0.04)", borderLeft: `3px solid ${STATUS_COLOR[w.status]}` }}>
              <div className="grid gap-2 px-4 py-3.5 lg:grid-cols-[auto_minmax(0,1fr)_auto_auto_auto_auto] lg:items-center lg:gap-4">
                ...
                <button onClick={() => setSelected(w)} className="rounded px-2 py-1 text-[11.5px] font-medium flex-shrink-0" style={{ border: "1px solid #E2E6EE", background: "#FFFFFF", color: "#56627A" }}>Edit</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {selected && <WebhookPanel webhook={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}

function WebhookPanel({ webhook, onClose }: { webhook: (typeof WEBHOOKS)[number]; onClose: () => void }) {
