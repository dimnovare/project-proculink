...
export const SECTION_GUIDES: SectionGuideEntry[] = [
  ...
  },
  {
    ...
    purpose: "Author this {supplier}'s mapping on the draft revision — drag source fields to the canonical spine to your output schema — then test, publish, and roll back. Every order pins the revision that processed it.",
    bullets: [
      { text: "On a draft, drag source fields → canonical spine → output fields to build the mapping" },
      { text: "AI proposes likely mappings as ghost wires you accept or reject" },
      { text: "Add your own canonical fields, and transforms or fixed values per output field" },
      { text: "The raw source bag is collapsed by default — search or expand it to find any field" },
      { text: "Preview the live output, run tests, then publish; published revisions are read-only" },
    ],
    firstStep: { text: "Create a draft from live, drag source → canonical → output to author the mapping, preview it, run tests, then publish." },
