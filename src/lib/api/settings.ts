...
export async function getSftpSettings(): Promise<SftpIngressSettings> {
  if (USE_MOCK) {
    await delay(120); // mock reads simulate latency — see getBillingStatus in api/billing.ts
    return { enabled: false, host: "", port: 22, username: "", remoteDirectory: "", defaultSupplierId: null, hasPassword: false, passwordDisplay: null, updatedAt: null };
  }
  const headers = await authHeader();
...
export async function getS3Settings(): Promise<S3IngressSettings> {
  if (USE_MOCK) {
    await delay(120); // mock reads simulate latency — see getBillingStatus in api/billing.ts
    return { enabled: false, bucketName: "", keyPrefix: "", region: "", accessKeyId: "", defaultSupplierId: null, hasSecretKey: false, secretKeyDisplay: null, updatedAt: null, serviceUrl: null };
  }
  const headers = await authHeader();
