...
const STATUS_BADGE: Record<string, { label: string; bg: string; color: string; border: string }> = {
  sent:                  { label: "Sent",                  bg: "#E3EDFB", color: "#0F4FA8", border: "#BBD3F4" },
...
function StatusBadge({ status }: { status: string }) {
  const meta = STATUS_BADGE[status] ?? { label: status, bg: "#EFF2F7", color: "#56627A", border: "#E2E6EE" };
  return (
  ...
  return (
    <tr style={{ borderTop: "1px solid #F0F2F6", background: rejected ? "#FDF1F1" : "transparent" }}>
      ...
      <td style={{ padding: "8px 8px", textAlign: "right", whiteSpace: "nowrap" }}>
        {rejected ? <span style={{ color: "#C53A3A" }}>—</span> : <Delta ordered={String(line.orderedQuantity ?? "—")} confirmed={String(line.confirmedQuantity ?? "—")} changed={qtyChanged} />}
      </td>
      <td style={{ padding: "8px 8px", textAlign: "right", whiteSpace: "nowrap" }}>
        {rejected ? <span style={{ color: "#C53A3A" }}>—</span> : <Delta ordered={money(currency, line.orderedUnitPrice)} confirmed={money(currency, line.confirmedUnitPrice)} changed={priceChanged} />}
      </td>
      <td style={{ padding: "8px 8px", textAlign: "right", whiteSpace: "nowrap" }}>
        {rejected ? <span style={{ color: "#C53A3A" }}>—</span> : <Delta ordered={fmtDate(line.orderedDeliveryDate)} confirmed={fmtDate(line.confirmedDeliveryDate)} changed={dateChanged} />}
      </td>
      <td style={{ padding: "8px 8px", textAlign: "right" }}>
        <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.04em", color: rejected ? "#C53A3A" : "#9A5F0A" }}>
...
function ConfirmationCard({ confirmation, currency }: { confirmation: SupplierConfirmation; currency: string | undefined }) {
  ...
  return (
    <div style={{ border: "1px solid #E2E6EE", borderRadius: 8, background: "#FFFFFF", overflow: "hidden" }}>
      {/* Header */}
      <div style={{ padding: "12px 14px", borderBottom: "1px solid #E2E6EE", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ padding: 14 }}>
        {confirmation.notes && (
          <p style={{ margin: "0 0 12px", fontSize: 12.5, color: "#56627A", lineHeight: 1.5 }}>{confirmation.notes}</p>
        )}

        <div style={{ fontSize: 11.5, color: "#56627A", marginBottom: changed.length > 0 ? 10 : 0 }}>
        ...
        {changed.length > 0 && (
          <div style={{ overflowX: "auto", border: "1px solid #E2E6EE", borderRadius: 6 }}>
...
export function SupplierResponsePanel({ orderId, currency }: { orderId: string; currency?: string }) {
  ...
  if (showLoading) {
    return (
      <div style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ height: 64, background: "#EFF2F7", borderRadius: 8 }} />
      </div>
    );
  }
  ...
  if (!confirmations || confirmations.length === 0) {
    return (
      <div style={{ border: "1px dashed #C6CDDA", borderRadius: 8, background: "#FFFFFF", padding: "20px 16px", display: "flex", alignItems: "center", gap: 12 }}>
        <StatusBadge status="no_response" />
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#0B1A2F" }}>No response yet</div>
          <div style={{ fontSize: 12, color: "#56627A", marginTop: 1 }}>
