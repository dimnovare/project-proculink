...
// revision that rendered nothing, in `SupplierConnectionService.cs`:
//
...
// and `src/test/replayImpactBackendMirror.test.ts` diffs the two against the real C#
// so they cannot drift apart the way the nine status vocabularies once did.
//
// Note the SECOND half of the backend predicate, which is the quieter half: with
// `OrderCount == 0` the backend passes WITH A NOTE ("No recent orders matched the
// replay window — pass-with-note"), because nothing was tested. The old `noImpact`
// silently graded that same state "safe to go live" — an empty result set and a
// clean result set were the same green.
...
export interface ReplayImpact {
   ...
   * branch sets it, and that branch requires `rendered > 0` — the backend's own
   * condition for letting the revision publish at all.
   */
  safeToGoLive: boolean;
}

/**
 * The backend's publish predicate, restated on the frontend's summary shape.
 *
 * `replayPassed == OrderCount == 0 || rendered > 0`. Kept as its own exported
 * function so the mirror test can evaluate it against a truth table rather than
 * string-matching a rendered sentence.
 */
export function backendWouldPassReplayLeg(s: Pick<ReplaySummary, "total" | "rendered">): boolean {
...
export function replayImpact(s: ReplaySummary): ReplayImpact {
  ...
  // Nothing was replayed. The backend calls this "pass-with-note" precisely because
  // passing on an empty set says nothing; saying "safe to go live" here would be
  // absence rendering as approval.
  if (s.total === 0) {
    return {
      tone: "warning",
      safeToGoLive: false,
  ...
  // Rendered for some, failed for others, and changed nothing where it did render.
  // "Would process the same way" is true of the survivors only, so it cannot lead.
  if (s.errors > 0) {
    return {
      tone: "warning",
      safeToGoLive: false,
