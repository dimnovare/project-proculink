 ...
 *
 * PRODUCT-LOOP ONLY (founder direction 2026-06-04): no marketing pages, and
 * NO page-scrolling — the camera never pans a page. We open in-app and drive
 * the loop by clicks; where a control is off-screen we use element-targeted
 * scrollIntoViewIfNeeded() (interaction/"element focus"), never window.scrollTo.
 *
 ...
 *   + AI mapping; accept each suggestion) → commit → order review → validate
 *   (Passed) → SEND (Generating… → Sent → Delivered) → Passport (audit + 200).
 *
...
test("ProcuLink walkthrough capture", async ({ page }) => {
  ...
  // ── s4 — validate: check the order against the supplier's acceptance rules ─
  await soft("s4.validate", async () => {
