...
- Continue Group I QA before starting Group J: full first-upload-to-delivery happy/error paths against a running API still need the same desktop/tablet/mobile pass. Group J should convert the local connector/webhook/mapping/rule/template QA affordances into real persistence/test-fire verification.
